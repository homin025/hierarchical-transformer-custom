...
from module.rouge import test_rouge, rouge_results_to_str
...
class Translator(object):
    ...
    def translate(self, data_iter, step):
        ...
        self.raw_src_out_file = codecs.open(raw_src_path, 'w', 'utf-8')
        ...
        with torch.no_grad():
            for batch in data_iter:
                ...
                for trans in translations:
                    ...
                    gold_str = ' '.join(gold).replace('<t>', '').replace('</t>', '').replace('<Q>', ' ').replace(r' +', ' ').strip()
                    ...
                    self.raw_src_out_file.write(src.strip() + '\n')
                ...
                self.raw_src_out_file.flush()

                if count > self.args.max_wiki:
                    break
