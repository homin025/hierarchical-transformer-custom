...
def load_dataset(args, vocab, corpus_type, shuffle):
    """
    ...
    dataset = []
    for document in js['document']:
        data = {'src': [], 'tgt': None, 'tgt_str': None}

        bodys = document['body']
        for body in bodys:
            _body = vocab.EncodeAsIds(body)
            data['src'].append(_body)

        abstract = ' '.join(document['abstract_sentences'])
        data['tgt'] = vocab.EncodeAsIds(abstract)
        data['tgt_str'] = abstract

        dataset.append(data)

    logger.info('Loading %s dataset from %s, number of examples: %d' %
                (corpus_type, args.data_path, len(dataset)))
...
class CustomDataLoader(object):
    def __init__(self, args, datasets, vocab, symbols, batch_size, device, shuffle, is_test):
        self.args = args
    ...
    def _next_dataset_iterator(self, dataset_iter):
        try:
            ...
            self.cur_dataset = next(iter(dataset_iter))
        except StopIteration:
            return None

        return CustomIterator(args=self.args,
...
class CustomIterator(object):
    ...
    def preprocess(self, ex):
        ...
        bop_id = self.symbols['BOP']
        eop_id = self.symbols['EOP']
        sum_id = self.symbols['SUM']
    ...
    def __iter__(self):
        while True:
            ...
            for idx, minibatch in enumerate(self.batches):
                ...
                batch = CustomBatch(minibatch, self.args.hier, self.symbols['PAD'], self.device, self.is_test)
                yield batch
            return
